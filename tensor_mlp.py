# -*- coding: utf-8 -*-
"""
Created on Wed Jun 17 15:45:41 2020
...
class ANN(object):
    ...
    def __int__(self,size,logPath,trainSet,validationSet,testSet,lambda_=1e-3):
        """
      创建一个神经网络
        """
        #重置tensorflow里的graph，以变多次运行
        tf.reset_default_graph()
        tf.set_random_seed(1908)
        self.logPath=logPath
        self.layerNum=len(size)
        self.size=size
        self.trianSet=trainSet
    ...
    def evaluation(self,epoch):
        ...
        """
        print("epoch %s" % epoch)
        #在windows系统下运行该脚本需要确保Windows下的cmd能显示中文
        print("训练集的准确率 %.3f" % self._doEval(self.trainSet["X"], self.trianSet["Y"]))
    ...
    def SGD(self,X,Y,startLearningRate, miniBatchFraction, epoch, keepProb):
        ...
        def fit(self,startLearningRate=0.1, miniBatchFraction=0.01, epoch=200,
                keepProb=0.7):
